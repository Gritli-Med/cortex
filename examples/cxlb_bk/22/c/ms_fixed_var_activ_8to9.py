...
# Title: Ms Fixed Var Activ 8To9
# Generated: Thu Feb  6 15:07:31 2020
##################################################
...
def argument_parser():
    ...
    parser.add_option(
        "", "--ar0", dest="ar0", type="eng_float", default=eng_notation.num_to_str(0.9),
        help="Set Activation rate [default=%default]")
    parser.add_option(
        "", "--ar1", dest="ar1", type="eng_float", default=eng_notation.num_to_str(1),
        help="Set Activation rate [default=%default]")
    parser.add_option(
        "", "--control0", dest="control0", type="string", default='8',
        help="Set Control [default=%default]")
    parser.add_option(
        "", "--control1", dest="control1", type="string", default='9',
        help="Set Control [default=%default]")
    return parser


def main(top_block_cls=ms_fixed_var_activ_8to9, options=None):
    if options is None:
        options, _ = argument_parser().parse_args()

    tb = top_block_cls(ar0=options.ar0, ar1=options.ar1, control0=options.control0, control1=options.control1)
    tb.start()
    tb.wait()
